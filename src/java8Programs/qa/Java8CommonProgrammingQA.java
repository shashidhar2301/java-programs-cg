...
import java8.master.qa.Employee;
...
public class Java8CommonProgrammingQA {
    ...
    public static void main(String[] args) {
        ...
        Map<Employee, Long> duplicates = employees.stream()
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()))
                .entrySet().stream()
                .filter(entry -> entry.getValue() > 1)
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
       duplicates.forEach((employee,count)-> System.out.println(employee));
        ...
        int[] result = Arrays.stream(nums).map(i -> i * i).sorted().toArray();
        ...
        Object[] combinedArray = Stream.concat(Arrays.stream(a).boxed(), Arrays.stream(b).boxed()).sorted().toArray();
