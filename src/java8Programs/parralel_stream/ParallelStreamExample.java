package java8Programs.parralel_stream;


import java.util.List;
import java.util.stream.IntStream;

import java8Programs.map_reduce.Employee1;
import java8Programs.map_reduce.EmployeeDatabase;
...
public class ParallelStreamExample {
    ...
    public static void main(String[] args) {
       ...
       IntStream.range(1,100).parallel().forEach(System.out::println);
        ...
        List<Employee1> employees = EmployeeDatabase.getEmployees();

        //normal
        start=System.currentTimeMillis();
        double salaryWithStream = employees.stream()
                .map(Employee1::getSalary).mapToDouble(i -> i).average().getAsDouble();
        end=System.currentTimeMillis();
